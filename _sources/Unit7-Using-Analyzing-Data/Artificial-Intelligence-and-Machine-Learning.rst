...
.. raw:: html
    ...
    <p><a href="https://quickdraw.withgoogle.com/" target="_blank"><img src="../_static/assets/img/FlagDoodle.png" style="float:left;" width="200"/></a>But AI researchers have developed an approach known as <i><b>machine learning</b></i> that enables computers to perform these complex tasks.  With machine learning a computer <i><b>learns</b></i> how to perform a task or solve a problem not by being given a traditional program to solve the problem, but by being given lots of examples of correct and incorrect solutions to the problem.  
    ...
    <p>Hopefully you enjoyed that activity and are curious now about machine learning.  The following short (7:48) tutorial was created by an undergraduate student from Texas A&amp;M University. It is one of many good tutorials that you can find online. 
...
.. raw:: html
    ...
    <p>
      As you learned, <i>machine learning</i> is based on an algorithm called <i>back propagation</i> that repeatedly adjusts the connections (weights) between the nodes in the <i>neural network</i>. To summarize, the machine learning algorithm consists of the following steps:
    </p><h4>Machine Learning Algorithm</h4>
    <ul>
    <li>Initialize the neural network by assigning random weights to connections between its nodes.</li>
    <li>Repeat until there is little to no difference between the network's actual outputs and the desired outputs.</li>
    <ul>
    <li>Present the neural network with a set of inputs.</li>
    <li>Compare the network's output with the desired output and calculate the difference (the error).</li>
    <li>Adjust the weights on the hidden nodes using <i>back propagation</i>.
        </li></ul>
    </ul>
    <p>
    ...
    <p>Don't be misled by this basic overview of simple neural networks and our simple explanation of machine learning.  Designing and implementing successful neural networks and machine learning algorithms to address problems like speech recognition (Siri, Alexa), computer vision (Face Recognition software), natural language understanding (Google Translate), and the many other tasks that are simple and natural for humans is an extremely complex and challenging research field. </p>
    <p>It has taken AI researchers many years of hard work to accomplish some of the examples you are seeing in this lesson.  In contrast to the simple networks shown in the video, current approaches AI are based on <i><b>deep learning</b></i> networks, which are networks that contain multiple layers in between the input and output layers. The additional <i>hidden</i> layers enable the network to learn abstract representations.</p>
    <p>Now, let's have some fun with AI by training our own neural network using Google's Teachable Machine software.</p>
...
.. raw:: html
    ...
    <p>As you saw in the video, you can train a simple neural network to distinguish between three classes: green, purple, and orange.  The network will associate a certain inputs from your video camera (hand up, funny face) with certain GIF images (cat, dog) or sounds (hello, hey). Once you've trained the network to make the desired associations, you have created a <i><b>model</b></i>. </p>
    ...
    <p>So, Google Translate got the translation wrong because it has allowed some bias to creep into its peformance.  While that was a fairly mild example of the type of bias that can occur, this has become a significant issue in machine learning.  Perhaps the most famous example of what can go very wrong was Micrsoft's disasterous experiment with Tay, its <a href="https://www.youtube.com/watch?v=Lr4yi9onykg" target="_blank">chatbot that was released on Twitter</a> and quickly learned a wide range of racist, sexist, and anti-semitic views.</p>
...
.. raw:: html
    ...
    <li>In this video two Googlers, Nat and Lo, interview a couple of Google AI researchers who describe <a href="https://www.youtube.com/watch?time_continue=1&amp;v=bHvf7Tagt18" target="_blank">how machine learning works</a>. This video was made as part of their "20% project".  One of the cool features of working at Google and other technology companies is that employees get to spend part of their time (1 day per week in this case) working on projects that they themselves choose.   
        </li>
    ...
    <li>Computer vision is a long-standing AI research area. In this TED talk, Wei-wei Li from Stanford University describes how she used machine learning and crowd source to to <a href="https://www.youtube.com/watch?time_continue=1&amp;v=tiwVMrTLUWg" target="_blank">teach a computer to understand pictures</a>.
        </li>
